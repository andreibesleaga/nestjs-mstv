...
describe('MicroserviceService Integration Tests', () => {
  ...
  describe('Message Publishing', () => {
    it('should publish Kafka events when enabled', async () => {
      ...
      expect(kafkaService.publishMessage).toHaveBeenCalledWith('user.created', eventData);
    });
    ...
    it('should handle Kafka publishing errors gracefully', async () => {
      ...
      await expect(service.sendKafkaMessage('user.created', { test: 'data' })).rejects.toThrow(
        'Kafka error'
      );
    });

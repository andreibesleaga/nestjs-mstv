...
describe('Performance and Load Tests', () => {
  ...
  describe('Concurrent Request Handling', () => {
    ...
    it('should handle mixed endpoint concurrent requests', async () => {
      ...
      expect(successfulResponses.length).toBeGreaterThanOrEqual(3); // Allow for some failures
  ...
  describe('Memory Usage and Resource Management', () => {
    it('should maintain stable memory usage during load', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        // Reduced from 100
        promises.push(request(app.getHttpServer()).get('/health').timeout(2000));
      }
      ...
      console.log(
        `Memory usage - Initial: ${Math.round(initialMemory.heapUsed / 1024 / 1024)}MB, Final: ${Math.round(finalMemory.heapUsed / 1024 / 1024)}MB`
      );
    });
    ...
    it('should handle large payloads efficiently', async () => {
      ...
      console.log(
        `Large payload (${JSON.stringify(largePayload).length} bytes) processed in ${responseTime}ms`
      );
    });
  });
  ...
  describe('Database Performance', () => {
    ...
    it('should handle rapid queries efficiently', async () => {
      ...
      const allowedStatuses = [200, 400, 401, 403];
      const allowedQueries = responses.filter((r) => allowedStatuses.includes(r.status));

      expect(totalTime).toBeLessThan(10000);
      expect(allowedQueries.length).toBeGreaterThanOrEqual(3);
  ...
  describe('Rate Limiting Performance', () => {
    it('should apply rate limiting without significant overhead', async () => {
      ...
      console.log(
        `Rate limiting test: ${successResponses.length} auth failures, ${rateLimitedResponses.length} rate limited in ${totalTime}ms`
      );
    });
  });
